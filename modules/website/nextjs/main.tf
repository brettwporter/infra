...
resource "aws_lambda_function" "image_optimization" {
  ...
  environment {
    variables = {
      BUCKET_NAME = aws_s3_bucket.asset_files.id
    }
  }
    ...
    Statement = [
      {
        ...
        Effect   = "Allow"
        Resource = "${aws_s3_bucket.asset_files.arn}/*"
      }
    ]
  })
...
resource "aws_s3_bucket" "asset_files" {
  bucket_prefix = var.asset_bucket_config.bucket_name
...
resource "aws_lambda_event_source_mapping" "revalidation_queue" {
  ...
  function_name    = aws_lambda_function.revalidation.function_name
}
...
resource "aws_s3_bucket" "cache" {
  bucket_prefix = var.cache_bucket_config.bucket_name
...
resource "aws_lambda_function" "warmer" {
  ...
  role             = aws_iam_role.warmer_lambda.arn
...
resource "aws_cloudwatch_event_rule" "warmer_cron" {
  name                = var.warmer_cron_eventbridge_event_rule_config.name
  schedule_expression = var.warmer_cron_eventbridge_event_rule_config.schedule_expression
}

resource "aws_cloudwatch_event_target" "warmer_eventbridge_cron_target" {
  arn  = aws_lambda_function.warmer.arn
  rule = aws_cloudwatch_event_rule.warmer_cron.name
}
...
resource "aws_cloudfront_distribution" "this" {
  ...
  # Asset Bucket Origin
  origin {
    domain_name = aws_s3_bucket.asset_files.bucket_regional_domain_name
    origin_id   = aws_s3_bucket.asset_files.id
    origin_path = "/assets"
  ...
  ordered_cache_behavior {
    ...
    target_origin_id       = aws_s3_bucket.asset_files.id
